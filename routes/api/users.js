...
router.get("/", async (req, res, next) => {
    ...
    if (req.query.search !== undefined) {
        searchObj = {
            $or: [{ firstName: { $regex: req.query.search, $options: "i" } }, { lastName: { $regex: req.query.search, $options: "i" } }, { username: { $regex: req.query.search, $options: "i" } }],
        };
    }
    User.find(searchObj)
...
router.put("/:userId/follow", async (req, res, next) => {
    ...
    User.findByIdAndUpdate(user._id, { [option]: { followers: req.session.user._id } }).catch((error) => {
        console.log(error);
        res.sendStatus(400);
    });
