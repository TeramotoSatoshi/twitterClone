...
router.post("/:id/retweet", async (req, res, next) => {
    ...
    // 既にリツイート済みの場合削除
    let deletedPost = await Post.findOneAndDelete({ postedBy: userId, retweetData: postId }).catch((error) => {
        console.log(error);
        res.sendStatus(400);
    });
    ...
    // ユーザーに値を追加する({$addToSet: {フィールド名: 追加する値}}}) -- 値の重複を許さない
    req.session.user = await User.findByIdAndUpdate(userId, { [option]: { retweets: rePost._id } }, { new: true }).catch((error) => {
        console.log(error);
        res.sendStatus(400);
    });

    // 投稿に値を追加する({$addToSet: {フィールド名: 追加する値}}) -- 値の重複を許さない
    let post = await Post.findByIdAndUpdate(postId, { [option]: { retweetUsers: userId } }, { new: true }).catch((error) => {
        console.log(error);
        res.sendStatus(400);
    });
